...
extern librdf_world* librdf_world_ptr;
...
- (GTWRedlandTripleStore*) initWithDictionary: (NSDictionary*) dictionary {
    return [self initWithName:dictionary[@"store_name"] redlandPtr:librdf_world_ptr];
}
...
- (BOOL) enumerateTriplesMatchingSubject: (id<GTWTerm>) s predicate: (id<GTWTerm>) p object: (id<GTWTerm>) o usingBlock: (void (^)(id<GTWTriple> t)) block error:(NSError **)error {
	...
	librdf_query* query	= librdf_new_query(librdf_world_ptr, "sparql", NULL, (unsigned const char*) [queryString UTF8String], NULL);
    if (!query) {
        if (error) {
...
- (BOOL) removeTriple: (id<GTWTriple>) t error:(NSError **)error {
    ...
    librdf_node *p  = [self objectToRaptorTerm:t.predicate];
    librdf_node *o  = [self objectToRaptorTerm:t.object];
    librdf_statement* st    = librdf_new_statement_from_nodes(librdf_world_ptr, s, p, o);
...
- (librdf_node*) objectToRaptorTerm: (id<GTWTerm>) term {
    if ([term conformsToProtocol:@protocol(GTWIRI)]) {
        return librdf_new_node_from_uri_string(librdf_world_ptr, (const unsigned char*) [[term value] UTF8String]);
    } else if ([term conformsToProtocol:@protocol(GTWLiteral)]) {
        GTWLiteral* l   = (GTWLiteral*) term;
        if (l.datatype) {
            librdf_uri* dt  = librdf_new_uri(librdf_world_ptr, (const unsigned char*) [[l datatype] UTF8String]);
            return librdf_new_node_from_typed_literal(librdf_world_ptr, (const unsigned char*) [[l value] UTF8String], NULL, dt);
        } else {
            return librdf_new_node_from_literal(librdf_world_ptr, (const unsigned char*) [[l value] UTF8String], [[l language] UTF8String], 0);
        }
    } else if ([term conformsToProtocol:@protocol(GTWBlank)]) {
        return librdf_new_node_from_blank_identifier(librdf_world_ptr, (const unsigned char*) [[term value] UTF8String]);
    } else {
        return nil;
    }
}
...
- (NSString*) description {
    ...
    librdf_serializer* s    = librdf_new_serializer(librdf_world_ptr, NULL, NULL, NULL);
    NSLog(@"librdf serializer: %p\n", s);
    librdf_uri* base        = librdf_new_uri(librdf_world_ptr, (const unsigned char*) "http://base/");
